...
########################################################################################################################
    ...
    def forward(self, image, context, inv_depths,
        ...
        for j, (ref_image, pose) in enumerate(zip(context, poses)):
            ...
            # pic_orig = image[0].cpu().clone()
            # pic_orig = (pic_orig.permute(1,2,0).numpy()*255).astype(np.uint8)
            # pic_ref = ref_image[0].cpu().clone()
            # pic_ref = (pic_ref.permute(1,2,0).numpy()*255).astype(np.uint8)
            # pic_warped = ref_warped[0][0].cpu().clone()
            # pic_warped = (pic_warped.permute(1,2,0).detach().numpy()).astype(np.uint8)
            # final = cv2.hconcat((pic_orig,pic_ref,pic_warped))
            # if j == 0:
            #     cv2.imshow('t-1', final)
            #     cv2.waitKey()
            # else:
            #     cv2.imshow('t+1', final)
            #     cv2.waitKey()

...
class SelfSupModel(SfmModel):
    ...
    def forward(self, batch, return_logs=False, progress=0.0):
        ...
        if not self.training:
            ...
            poses_gt = [[], []]
            poses_gt[0] = Pose(batch['pose_context'][0].to(torch.float32)).inverse()
            poses_gt[1] = Pose(batch['pose_context'][1].to(torch.float32)).inverse()
            ...
            self_sup_output = self.self_supervised_loss(
                ...
                output['inv_depths'], poses_gt, batch['intrinsics'],
                return_logs=return_logs, progress=progress)

...
class HorovodTrainer(BaseTrainer):
    def __init__(self, **kwargs):
        ...
        self.writer_idx_train = 0
        self.writer_idx_validate = 0
    ...
    def train(self, dataloader, module, optimizer):
        ...
        for i, batch in progress_bar:
            ...
            # Adding tensorboard monitoring
            self.writer.add_scalar('Loss/train_loss'
                                   , output['loss']
                                   , self.writer_idx_train)
            self.writer.add_scalar('Loss/photometric_loss'
                                   , output['metrics']['photometric_loss'].detach()
                                   , self.writer_idx_train)
            # self.writer.add_scalar('Loss/supervised_loss',
            #                        output['metrics']['supervised_loss']
            #                        , self.writer_idx_train)
            self.writer_idx_train += 1
            # self.writer.add_scalar('Loss/smoothness_loss',
            #                        output['metrics']['smoothness_loss'].detach()
            #                        , self.writer_idx_train)

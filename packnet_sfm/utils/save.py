...
def save_depth(batch, output, args, dataset, save):
    ...
    if save.depth.rgb or save.depth.viz or save.depth.npz or save.depth.png:
        ...
        length = rgb.shape[0]
        filename_num = [os.path.split(filename[i])[1] for i in range(length)]
        # For each image in the batch

        for i in range(length):
            # Save numpy depth maps
            os.makedirs(os.path.join(save_path,os.path.split(filename[i])[0]).replace('{}',''), exist_ok=True)
            save_path = os.path.join(save_path,os.path.split(filename[i])[0]).replace('{}','')
            if save.depth.npz:
                write_depth('{}/{}_depth.npz'.format(save_path, filename_num[i]),
                            depth=inv2depth(pred_inv_depth[i]),
                            intrinsics=batch['intrinsics'][i] if 'intrinsics' in batch else None)
            # Save png depth maps
            if save.depth.png:
                write_depth('{}/{}_depth.png'.format(save_path, filename_num[i]),
                            depth=inv2depth(pred_inv_depth[i]))
            # Save rgb images
            if save.depth.rgb:
                rgb_i = rgb[i].permute(1, 2, 0).detach().cpu().numpy() * 255
                write_image('{}/{}_rgb.png'.format(save_path, filename_num[i]), rgb_i)
            # Save inverse depth visualizations
            if save.depth.viz:
                viz_i = viz_inv_depth(pred_inv_depth[i]) * 255
                write_image('{}/{}_viz.png'.format(save_path, filename_num[i]), viz_i)
